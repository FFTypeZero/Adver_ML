...
conv3 = tf.contrib.layers.conv2d(conv2_pad, num_outputs=96, kernel_size = [3, 3], stride = [2, 2], padding = PADDING_STRATEGY)
conv3_pad = tf.pad(conv3, PADDING_TENSOR)
drop1 = tf.contrib.layers.dropout(conv3_pad, keep_prob=0.5, is_training = if_drop)
...
conv5 = tf.contrib.layers.conv2d(conv4_pad, num_outputs=192, kernel_size=[3, 3], padding = PADDING_STRATEGY)
conv5_pad = tf.pad(conv5, PADDING_TENSOR)
conv6 = tf.contrib.layers.conv2d(conv5_pad, num_outputs=192, kernel_size=[3, 3], stride = [2, 2], padding = PADDING_STRATEGY)
conv6_pad = tf.pad(conv6, PADDING_TENSOR)
drop2 = tf.contrib.layers.dropout(conv6_pad, keep_prob=0.5, is_training = if_drop)
...
conv8 = tf.contrib.layers.conv2d(conv7_pad, num_outputs=192, kernel_size=[1, 1], padding = PADDING_STRATEGY)
