...
def obj_func(model, target_labels, confidence, targeted):
    ...
    # other = tf.reduce_sum((1 - target_labels)*lo, axis=1)
...
def l2_attack(model, sess, cons, lr, X, target, num_labels, confidence = 0, targeted = True, max_ite = 3000, tol = 5e-5):
    onehot_t = sess.run(tf.one_hot(target, depth = num_labels))
    shape = X.shape
